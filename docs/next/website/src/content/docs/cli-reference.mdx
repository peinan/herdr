...
herdr pane mark [<pane_id>|--pane ID|--current] --on|--off
...
herdr agent mark <target> [--on|--off|--toggle]
...
`agent mark` flags an agent for follow-up so it is easy to find after answering another one. Marked agents lead the sidebar under `agent_panel_sort = "priority"`, and `agent get` and `agent list` report the flag as `marked`. Marks live in the running server only and are not restored across restarts.

`pane mark` sets the same flag on a pane by ID, whether or not an agent is running in it, and `pane get` reports it as `marked`. A pane that later hosts an agent keeps the flag, so the agent surfaces show it. Only agent rows draw the sidebar marker, so marking a plain shell pane has no visible effect until an agent appears there.
