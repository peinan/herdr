...
`agent mark` and `pane mark` flag an agent for follow-up so it is easy to find after answering another one. Marked agents lead the sidebar under `agent_panel_sort = "priority"`, and `agent get` and `agent list` report the flag as `marked`. Marks live in the running server only and are not restored across restarts.
