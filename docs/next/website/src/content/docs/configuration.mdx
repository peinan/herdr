...
Set `single_pane_border = true` to keep a border around the pane even when a tab has only one pane. By default a lone pane has no border. This setting has no effect when `pane_borders = false`.
...
`zoom_indicator` is the marker shown in the zoomed pane's border title when you zoom a pane with `prefix+z`. It defaults to `Z` and is appended after the pane's existing label (or shown alone when the pane has no label). Set it to an empty string to hide the marker.
