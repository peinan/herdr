...
**Not available in this build:** repeatable prefix bindings are pending re-implementation on the client shell after the upstream 0.9.0 sync. The table form and `repeat_timeout` are still accepted so existing configs keep loading, but bindings behave as if `repeat = false`.
