...
A prefix binding can use a table form to opt in to tmux `bind -r` style repeat, so the bare key repeats the action while you stay armed in prefix mode:

```toml
[keys]
swap_pane_up = { key = "prefix+shift+k", repeat = true }
next_tab = { key = "prefix+n", repeat = true }
# Armed window in milliseconds. Default: 500. `0` clamps to the default.
repeat_timeout = 500
```

Repeat is off by default; string and array bindings never repeat. The armed state ends on the prefix key, `Esc`, an unbound key, or after `repeat_timeout` milliseconds. Repeat applies to navigation and cycling actions that return you to the terminal or navigate view — next/previous workspace, agent, and tab; swap, cycle, and focus pane; zoom; and toggle sidebar. It does not hold you armed for actions that open a dialog or enter a sub-mode (rename, help, settings, the workspace picker, the navigator, copy or resize mode); those transition as usual even if flagged `repeat = true`. See the [keyboard guide](/docs/preview/keyboard/#repeat-a-binding-without-re-pressing-the-prefix) for details.
...
`pane_title_format` customizes the text drawn in a pane's border title. It uses a starship-style subset: literal text, `$var` (or `${var}`) variables, and conditional groups `( … )` that disappear entirely when every variable inside them resolves to nothing. Available variables are `$dir` (cwd basename), `$cwd` (full cwd, `$HOME` shown as `~`), `$process` (the real foreground process, with shell aliases and wrappers resolved), `$agent`, `$branch` (the short commit when HEAD is detached), `$ahead_behind` (for example `⇡2⇣1`), `$git_status` (working-tree marks: `=` conflict, `!` modified, `+` staged, `?` untracked), `$zoom`, and `$label`. Escape the metacharacters with `\$`, `\(`, `\)`, and `\\`. For example, `pane_title_format = "$dir $process( ⋅ $branch$ahead_behind$git_status)( $zoom)"` shows the directory and process, a git section that is omitted outside a repository, and the zoom marker only while the pane is zoomed. An OSC-set terminal title or a manual pane name still takes precedence over the format, and an empty value (the default) keeps the built-in label. When a format is set it owns the title — including the zoom marker, which `$zoom` places (the indicator is auto-appended only in the default, no-format mode). An invalid format is ignored and reported as a config diagnostic.

`popup_padding` and `popup_title_format` style the popup pane — the session-modal terminal a `type = "popup"` keybinding or a plugin entrypoint opens — and are separate from the pane keys so that padding a pane does not silently reshape every popup. `popup_padding` takes the same per-side table as `pane_padding` and insets the popup's terminal area from its border. `popup_title_format` uses the same syntax as `pane_title_format`, but a popup is not part of a workspace layout: it resolves `$dir` and `$cwd` from the cwd it inherited when it opened, resolves `$branch`, `$ahead_behind` and `$git_status` while some pane shares the repository, and leaves `$process`, `$agent` and `$zoom` empty so conditional groups built from them drop out. A popup opened with an explicit name — a plugin entrypoint title — keeps that name; otherwise an empty format (the default) keeps the built-in `popup` label. However the label is chosen, the border draws it with a space on either side and truncates it with `…` when the popup is too narrow to hold it. An invalid format is ignored and reported as a config diagnostic.

`popup_background` decides how much of the popup gets the panel background painted behind it. The default `panel` covers the border and the padding and leaves the content area showing whatever the program inside the popup draws. `all` additionally fills in behind content the program left on the terminal's default background, so the popup reads as solid over a transparent terminal. `transparent` covers none of it, keeping a transparent terminal transparent around the popup rather than showing an opaque frame over it. A program that chooses its own background — a full-screen TUI, a syntax-highlighted pager — always keeps it, in every mode. Use `theme.custom.panel_bg` instead if you want every panel, overlay and toast transparent, not just popups.

`popup_dim_background` dims what is behind an open popup so the modal reads as modal. The default `off` leaves the background at full brightness; `all` dims everything outside the popup, sidebar and tab bar included; `panes` dims only the pane area and leaves the chrome alone. The popup itself is never dimmed, and a modal overlay opened on top still dims the popup with it.

Set `sidebar_divider = false` to hide the vertical divider line between the sidebar and the main pane area.

`zoom_indicator` is the marker shown in the zoomed pane's border title when you zoom a pane with `prefix+z`. It defaults to `Z` and is appended after the pane's existing label (or shown alone when the pane has no label). Set it to an empty string to hide the marker. `zoom_indicator_position` selects where it appears: `pane` or `both` draw it in the pane border title and `none` hides it; the tab-label position (`tab`, the default, and the tab half of `both`) is pending re-implementation on the client shell after the upstream 0.9.0 sync and currently draws nothing.

`prefix_indicator` controls how Herdr signals that the prefix key is pending. The default `status_bar` shows the bottom hint bar (`esc cancel`, `send prefix`, and so on). Set it to `highlight` to hide that bar and instead recolor the focused pane border and the active tab, giving a quieter prefix-mode cue at the cost of the inline key hints.
