...
`pane_title_format` customizes the text drawn in a pane's border title. It uses a starship-style subset: literal text, `$var` (or `${var}`) variables, and conditional groups `( … )` that disappear entirely when every variable inside them resolves to nothing. Available variables are `$dir` (cwd basename), `$cwd` (full cwd, `$HOME` shown as `~`), `$process`, `$agent`, `$branch`, `$ahead_behind` (for example `↑2↓1`), `$git_status`, `$zoom`, and `$label`. Escape the metacharacters with `\$`, `\(`, `\)`, and `\\`. For example, `pane_title_format = "$dir $process( ⋅ $branch$ahead_behind$git_status)"` shows the directory and process, plus a git section that is omitted outside a repository. An OSC-set terminal title or a manual pane name still takes precedence over the format, and an empty value (the default) keeps the built-in label. An invalid format is ignored and reported as a config diagnostic.
