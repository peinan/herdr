...
`pane.mark.set` 为一个窗格标记待办。`PaneInfo` 和 `AgentInfo` 以 `marked` 报告该标记，智能体视图可以按内置字段 `marked` 过滤和排序，在 `agent_panel_sort = "priority"` 下已标记的智能体排在侧边栏最前。该标记只保存在运行中的服务器上，重启后不会恢复。
