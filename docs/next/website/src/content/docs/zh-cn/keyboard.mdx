...
有些前缀动作天然适合连续触发多次,比如逐个切换标签页、轮换窗格,或把窗格在布局中挪动。你可以把绑定标记为可重复,而不必在每次按键前都先按前缀键。按下 `prefix` + 该键之后,Herdr 会保持“待命”状态,你可以直接再按裸键来重复执行,直到你按下其他任何键或短暂的超时结束。这与 tmux 的 `bind -r` 一致。

使用表格形式并设置 `repeat = true`:

```toml
[keys]
# 按下 `prefix+shift+k` 之后,持续按 `shift+k` 就会不断把窗格向上交换。
swap_pane_up = { key = "prefix+shift+k", repeat = true }
swap_pane_down = { key = "prefix+shift+j", repeat = true }
# 按下 `prefix+n` 之后,持续按 `n` 就会逐个切换标签页。
next_tab = { key = "prefix+n", repeat = true }
previous_tab = { key = "prefix+p", repeat = true }

# 待命窗口的毫秒数。默认: 500。`0` 会回落到默认值。
repeat_timeout = 500
```

只要你按下前缀键、`Esc` 或未绑定的键,或者 `repeat_timeout` 毫秒内没有任何输入,待命状态就会结束。重复是可选功能,默认关闭;普通的字符串和数组形式绑定永远不会重复。

重复适用于导航和轮换类动作,也就是执行后会让你回到终端或导航视图的那些:上一个/下一个工作区、代理和标签页;交换、轮换和聚焦窗格;缩放;以及切换侧边栏。对于会打开对话框或进入子模式的动作,它**不会**保持待命——重命名、帮助、设置、工作区选择器、导航器,以及复制或调整大小模式都会照常切换并结束重复窗口,即使你设置了 `repeat = true`。给它们标记可重复没有实际效果,所以不要为它们开启。
