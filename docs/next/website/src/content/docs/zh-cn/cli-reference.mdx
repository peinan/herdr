...
`agent mark` 为智能体标记待办，便于在回复了另一个智能体之后再找回来。在 `agent_panel_sort = "priority"` 下，已标记的智能体排在侧边栏最前，`agent get` 和 `agent list` 会以 `marked` 报告该标记。标记只保存在运行中的服务器上，重启后不会恢复。

`pane mark` 按窗格 ID 设置同一个标记，无论该窗格中是否运行着智能体，`pane get` 会以 `marked` 报告。之后才启动智能体的窗格同样保留该标记，因此会反映到智能体相关的显示上。只有智能体行会绘制侧边栏标记，所以为普通 shell 窗格加标记在智能体出现之前没有可见变化。
