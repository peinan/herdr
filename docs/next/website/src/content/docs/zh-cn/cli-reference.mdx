...
`agent mark` 和 `pane mark` 为智能体标记待办，便于在回复了另一个智能体之后再找回来。在 `agent_panel_sort = "priority"` 下，已标记的智能体排在侧边栏最前，`agent get` 和 `agent list` 会以 `marked` 报告该标记。标记只保存在运行中的服务器上，重启后不会恢复。
