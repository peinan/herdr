...
`pane.mark.set` は 1 つのペインに後回しの目印を付けます。`PaneInfo` と `AgentInfo` は `marked` として報告し、エージェントビューは組み込みフィールド `marked` でフィルタ・ソートでき、`agent_panel_sort = "priority"` ではマーク済みがサイドバーの先頭に並びます。このフラグは実行中のサーバーにのみ保持され、再起動後には復元されません。
