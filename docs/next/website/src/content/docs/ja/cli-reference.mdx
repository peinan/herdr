...
`agent mark` と `pane mark` はエージェントに後回しの目印を付けます。別のエージェントに応答した後でも戻る先を見失いません。`agent_panel_sort = "priority"` ではマーク済みがサイドバーの先頭に並び、`agent get` と `agent list` は `marked` として報告します。マークは実行中のサーバーにのみ保持され、再起動後には復元されません。
