...
`agent mark` はエージェントに後回しの目印を付けます。別のエージェントに応答した後でも戻る先を見失いません。`agent_panel_sort = "priority"` ではマーク済みがサイドバーの先頭に並び、`agent get` と `agent list` は `marked` として報告します。マークは実行中のサーバーにのみ保持され、再起動後には復元されません。

`pane mark` は同じフラグをペイン ID 指定で設定します。そのペインでエージェントが動いているかどうかは問いません。`pane get` が `marked` として報告します。後からエージェントが起動したペインもフラグを保持するため、エージェント側の表示に反映されます。サイドバーのマーカーを描くのはエージェント行だけなので、素のシェルペインにマークを付けてもエージェントが現れるまで見た目には変化しません。
