...
| 単独ペインの枠 | `ui.single_pane_border` | ペインが 1 つだけでも枠を描く | 2026-06-27 | [#11](https://github.com/peinan/herdr/pull/11) / [`47a97f9`](https://github.com/peinan/herdr/commit/47a97f9) |
| 非アクティブペイン減光 | `ui.dim_inactive_panes` | 非フォーカスのペインを薄くしてアクティブを強調。**既定 off**(アップストリーム一致: prefix/コマンド時のみ減光)、`true` で常時減光 | 2026-06-26 | [#2](https://github.com/peinan/herdr/pull/2) / [`321db15`](https://github.com/peinan/herdr/commit/321db15), [#22](https://github.com/peinan/herdr/pull/22) / [`84a52ef`](https://github.com/peinan/herdr/commit/84a52ef) |
...
| サイドバー区切り線 | `ui.sidebar_divider` | サイドバー/ペイン領域の縦区切り線の表示切替 | 2026-06-26 | [#3](https://github.com/peinan/herdr/pull/3) / [`46a8de6`](https://github.com/peinan/herdr/commit/46a8de6) |
| ズームマーカー | `ui.zoom_indicator` / `ui.zoom_indicator_position` | `prefix+z` ズーム時のマーカー文字と表示位置(`tab`/`pane`/`both`/`none`、**既定 `tab`**) | 2026-06-26 | [#4](https://github.com/peinan/herdr/pull/4) / [`ffcdab9`](https://github.com/peinan/herdr/commit/ffcdab9), [#25](https://github.com/peinan/herdr/pull/25) / [`da3dbd8`](https://github.com/peinan/herdr/commit/da3dbd8) |
| タブバースタイル | `ui.tab_bar_{style,position,align,title}` | `classic`(上・テキスト・スクロール可)/`minimal`(グリフ帯)を選択、**既定 `classic`**。`minimal` は position/align/title で調整 | 2026-06-27 | [#10](https://github.com/peinan/herdr/pull/10) / [`c19f18b`](https://github.com/peinan/herdr/commit/c19f18b), [#24](https://github.com/peinan/herdr/pull/24) / [`68fe136`](https://github.com/peinan/herdr/commit/68fe136) |
| prefix 待機表示 | `ui.prefix_indicator` | prefix 待機の示し方(バー or 枠+タブ強調) | 2026-06-27 | [#5](https://github.com/peinan/herdr/pull/5) / [`aa108e7`](https://github.com/peinan/herdr/commit/aa108e7) |
| repeatable prefix バインド | `keys.*` / `keys.repeat_timeout` | tmux `bind -r` 風に素キーで繰り返し | 2026-06-26 | [#1](https://github.com/peinan/herdr/pull/1) / [`4874e19`](https://github.com/peinan/herdr/commit/4874e19) |
...
| `ui.dim_inactive_panes` | bool | `false` | 非フォーカスのペインを減光。既定 `false`=アップストリーム挙動(prefix/コマンドモード時のみ減光)、`true`=常時減光 |
...
| `ui.zoom_indicator_position` | enum | `"tab"` | ズームマーカーの表示位置。`tab`(既定)/`pane`/`both`/`none`。`pane_title_format` の `$zoom` もこれに従う。文字が空 or `none` で非表示 |
| `ui.tab_bar_style` | enum | `"classic"` | タブバーのスタイル。`classic`=上部テキストタブ(スクロール可、アップストリーム)/`minimal`=グリフ帯 |
| `ui.tab_bar_position` | enum | `"bottom"` | `minimal` のときのタブバー位置。`top`/`bottom`。`classic` では無効(常に上) |
...
タブバーは 2 系統から選べる。既定は `classic`(アップストリーム相当)。

```toml
[ui]
tab_bar_style    = "minimal"   # classic | minimal
tab_bar_position = "bottom"    # minimal のみ: top | bottom
...
- **現行フォークの見た目に戻すには** 個人 `config.toml` に `tab_bar_style = "minimal"` を書く
  (既定の `position=bottom` / `align=right` / `title=false` で従来の下端・右寄せ・グリフのみになる)。
- ズームマーカーをタブに出す場合(`zoom_indicator_position` が `tab`/`both`)、`classic` は常に、
  `minimal` は `tab_bar_title = true` のときだけタブに付く(グリフのみのタブには出さない)。
