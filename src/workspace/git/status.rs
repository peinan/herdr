...
pub struct GitStatusRefreshDemand {
    ...
    /// Working-tree dirty counts from `git status --porcelain`, read by the
    /// pane title's `$git_status`. Recomputed on every refresh while demanded
    /// because dirty edits do not move the oids the fingerprint gates on.
...
impl GitStatusRefreshDemand {
    ...
    pub fn is_empty(self) -> bool {
        !self.branch && !self.ahead_behind && !self.working_tree
    }
}
...
pub fn git_status_snapshot_for_cwd_with_demand(
    ...
    if !demand.ahead_behind {
        ...
        let snapshot = WorkspaceGitStatusSnapshot {
            ...
            ahead_behind: None,
            space: Some(space),
            // Dirty edits do not move the oids the fingerprint gates on, so
            // the working tree is recomputed on every refresh while demanded.
            working_tree: if demand.working_tree {
                git_working_tree_for_cwd(cwd)
            } else {
                GitWorkingTree::default()
            },
            short_commit: fingerprint
                .as_ref()
                .and_then(GitStatusFingerprint::short_commit),
        };
        return (
            snapshot.clone(),
    ...
    let working_tree = if demand.working_tree {
        git_working_tree_for_cwd(cwd)
    } else {
        GitWorkingTree::default()
    };
...
";
    ...
    fn git_status_short_commit_is_none_without_repo() {
        ...
        // Non-Git cwds are cached without a fingerprint so the miss is retried later.
        assert!(cache_entry.is_some_and(|entry| entry.fingerprint.is_none()));
        std::fs::remove_dir_all(root).unwrap();
    }
}
