...
/// the retained fast path matches the complete renderer's dimming. This is the
/// same bit `protocol::wire::modifier_to_u16` emits for the full path.
...
impl HeadlessServer {
    ...
    pub(super) fn render_retained_pane_surface_and_stream(
        ...
        for source in pty_sources {
            ...
            let mut patch = match runtime.collect_dirty_patch(width, height) {
            ...
            // The complete renderer dims unfocused panes while `dim_inactive_panes`
            // is on (`ui::panes::render_panes`); this fast path patches raw terminal
            // cells, so it must re-apply the same dim or a busy unfocused pane
            // flickers bright<->dim as the two paths alternate.
            if self.app.state.dim_inactive_panes
                && self
                    .app
                    .state
                    .workspaces
...
mod tests {
    ...
    fn apply_dim_to_patch_adds_dim_and_preserves_modifiers() {
        ...
        let mut bold_cell = cell(" ");
        bold_cell.modifier = bold;
        let mut patch = crate::pane::TerminalDirtyPatch {
            rows: vec![(0, vec![cell(" "), bold_cell])],
        };
    ...
    /// The retained fast path ORs `Modifier::DIM.bits()` directly, while the
    /// complete renderer adds `Modifier::DIM` and serializes via `modifier_to_u16`.
    /// Guard that both produce the same bit so the two paths never disagree.
    #[test]
