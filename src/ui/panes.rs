...
///
/// With `dim_inactive_panes` enabled, non-focused panes dim while another pane
/// has focus in a multi-pane layout (an always-on active-pane highlight).
fn should_dim_pane(is_focused: bool, multi_pane: bool, dim_inactive_panes: bool) -> bool {
    dim_inactive_panes && !is_focused && multi_pane
}
...
pub(super) fn render_panes(
    ...
    let multi_pane = pane_infos.len() > 1;
    ...
    for info in pane_infos {
        if let Some(rt) = app.runtime_for_pane_in_workspace(terminal_runtimes, ws_idx, info.id) {
            ...
            if should_dim_pane(info.is_focused, multi_pane, app.dim_inactive_panes) {
...
mod tests {
    use super::*;
    use crate::app::Mode;
    use crate::config::{PaneBordersConfig, ZoomIndicatorPosition};
    ...
    fn dim_inactive_panes_controls_unfocused_dimming() {
        ...
        assert!(!should_dim_pane(true, true, true));
        // Single-pane layouts never dim.
        assert!(!should_dim_pane(false, false, true));
        // Enabled: unfocused panes dim as an always-on active-pane highlight.
        assert!(should_dim_pane(false, true, true));
        // Disabled (default): upstream behavior, no dim.
