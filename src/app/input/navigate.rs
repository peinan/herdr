...
impl App {
    ...
    pub(crate) fn handle_prefix_key(&mut self, raw_key: TerminalKey) {
        ...
        if let Some((action, repeatable)) =
            non_indexed_action_for_key_with_repeat(&self.state, &raw_key, BindingDispatch::Prefix)
        {
            self.execute_prefix_key_action(action, repeatable);
            return;
        }
        ...
        {
            self.execute_prefix_key_action(action, false);
            return;
        }
    ...
    fn execute_prefix_key_action(&mut self, action: NavigateAction, repeatable: bool) {
...
fn non_indexed_action_for_key(
    ...
    non_indexed_action_for_key_with_repeat(state, key, dispatch).map(|(action, _repeatable)| action)
}

/// Like [`non_indexed_action_for_key`], but also reports whether the matched
/// binding opted in to repeat. `repeatable` is only ever `true` for prefix
/// bindings; indexed and direct matches always report `false`.
fn non_indexed_action_for_key_with_repeat(
    state: &AppState,
    key: &TerminalKey,
    dispatch: BindingDispatch,
...
fn finish_action_context(state: &mut AppState, context: ActionContext, previous_mode: Mode) {
    if context.is_repeatable_prefix() {
        ...
        // ConfirmClose, ...) keep that mode and exit the repeat window. `Copy`
        // needs the explicit exclusion: it doubles as the return-to base when
        // the focused pane is in copy mode (see `command_base_mode`), but
        // entering it still ends the repeat window.
        if state.mode == Mode::Prefix
            || (state.mode != Mode::Copy && state.mode == command_base_mode(state))
        {
...
/// The base mode prefix/command dispatch returns to: `Copy` when the focused
/// pane is in copy mode, `Terminal` when a pane is active, else `Navigate`.
/// Shared by `leave_command_mode` and the repeatable re-arm decision, except
/// that the re-arm path never treats `Copy` as re-armable — entering copy mode
/// ends the repeat window (see `finish_action_context`).
